...
const QuizContent = ({ isSidebarOpen, toggleSidebar, selectedTech, selectedLevel, currentQuestion, setCurrentQuestion, userAnswers, setUserAnswers, showResults, setShowResults, submittedRef, getQuestions, calculateScore, handleLevelSelect }) => {
    ...
    const submitResult = useCallback(async () => {
        ...
        if (!techObj || !levelObj) return;

        const payload = {
            title: `${techObj.name.toUpperCase()} - ${levelObj.name} quiz`,
    ...
    }, [selectedTech, selectedLevel, score.total, score.correct, submittedRef]);
                ...
                <div className={sidebarStyles.mobileTitle}>
                    {currentTech ? (
                        <div className={sidebarStyles.mobileTechInfo}>
                            <div
                                className={`${sidebarStyles.mobileTechIcon} ${currentTech.color}`}
                            >
                                {currentTech.icon}
                            </div>
                            <div className={sidebarStyles.mobileTechText}>
                                ...
                                <div className={sidebarStyles.mobileTechLevel}>
                                    {currentLevel ? `${currentLevel.name} level` : "Select level"}
                                </div>
                            </div>
                        </div>
                    ) : (
            ...
            {!selectedTech ? (
                ...
                <div className={sidebarStyles.levelSelectionContainer}>
                    <div className={sidebarStyles.levelSelectionContent}>
                        <div
                            className={`${sidebarStyles.techSelectionIcon} ${currentTech.color}`}
                        >
                            {currentTech.icon}
                        </div>
                        <h2 className={sidebarStyles.techSelectionTitle}>
                            {currentTech.name} Quiz
                        </h2>
                    ...
                    <div className={sidebarStyles.resultsContent}>
                        <div className={sidebarStyles.resultsHeader}>
                            ...
                            <p className={sidebarStyles.resultsSubtitle}>
                                You've completed the {currentLevel.name} level
                            </p>
                            <div
